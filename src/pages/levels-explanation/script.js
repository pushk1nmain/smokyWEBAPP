...
async function navigateToNextScreen() {
    ...
    try {
        ...
        if (window.StepRouter) {
            ...
            if (!success) {
                console.warn('⚠️ StepRouter не смог выполнить переход (возможно, это последний доступный экран)');
                console.log('ℹ️ Показываем сообщение о завершении или возвращаемся на главную');
