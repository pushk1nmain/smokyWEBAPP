...
class StepRouter {
    ...
    static STEP_TO_SCREEN = {
        ...
        19: '/src/pages/recognition-quiz/index.html',      // Узнали себя? (квиз узнавания)
        20: '/src/pages/transformation-lessons/index.html', // Уроки трансформации (поддержка)
        21: '/src/pages/levels-explanation/index.html',   // Объяснение уровней зависимости
    ...
    static STEP_NAMES = {
        ...
        19: 'Узнали себя?',
        20: 'Уроки трансформации',
        21: 'Объяснение уровней зависимости',
    };
    ...
    async getCurrentStep() {
        ...
        try {
            ...
            if (userData && typeof userData.progress_step === 'number') {
                ...
                // Убрали ограничения по максимальному количеству шагов
                // Позволяем пользователю находиться на любом шаге
                console.log(`📍 Пользователь на шаге ${userStep}, максимальный определенный шаг: ${maxStep}`);
            ...
            // При ошибке API сохраняем текущий шаг вместо сброса к 1
            console.error('❌ Ошибка при получении текущего шага:', error);
            
            // Пытаемся получить шаг из локального хранилища
            const localStep = parseInt(localStorage.getItem('currentStep'));
            if (localStep && localStep > 0) {
                console.log(`💾 Используем сохраненный локально шаг: ${localStep}`);
                this.currentStep = localStep;
                return localStep;
            }
            
            // Только для новых пользователей (404) начинаем с 1
            if (error.message?.includes('404') || error.message?.includes('не найден')) {
                console.log('👋 Новый пользователь, начинаем с первого шага');
                return 1;
            }
            
            // Для других ошибок (сеть, таймаут) возвращаем последний известный шаг
            console.warn('⚠️ Не удалось получить текущий шаг, используем последний сохраненный в классе');
    ...
    async updateStep(newStep) {
        // Сохраняем шаг локально для отката при ошибках
        ...
        try {
            ...
            if (result && result.success) {
                ...
                // При ошибке сервера все равно обновляем локально
                this.currentStep = newStep;
                console.log(`⚠️ Обновили шаг локально: ${newStep}`);
                return true;
            }
            ...
            // При ошибке сети все равно обновляем локально
            this.currentStep = newStep;
            console.log(`⚠️ Обновили шаг локально при ошибке: ${newStep}`);
            return true;
        }
    }
