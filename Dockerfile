...
CMD /bin/sh -c "if [ -z \"$API_KEY\" ]; then echo 'FATAL: API_KEY environment variable is not set'; exit 1; fi && envsubst < /etc/nginx/nginx.conf.template > /etc/nginx/nginx.conf && nginx -g 'daemon off;' "